...
namespace vroom.Areas.Identity.Pages.Account
{
    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task OnGetAsync(string returnUrl = null)
        {
            ...
            _logger.LogInformation("OnGetAsync: " + returnUrl);
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    if (!await _roleManager.RoleExistsAsync("Admin"))
                    {
                        ...
                        _logger.LogInformation("Create Admin: " + Input.UserName);
                    }
                    if (!await _roleManager.RoleExistsAsync("Executive"))
                    {
                        await _roleManager.CreateAsync(new IdentityRole("Executive"));
                        _logger.LogInformation("Create Executive: " + Input.UserName);
                    }
                    ...
                    if (Input.IsAdmin)
                    {
                        ...
                        _logger.LogInformation("AddUserToRoleAsync Admin: " + Input.UserName);
                    }
                    else
                    {
                        await _userManager.AddToRoleAsync(user, "Executive");
                        _logger.LogInformation("AddUserToRoleAsync Executive: " + Input.UserName);
                    }
                    ...
                    _logger.LogInformation("pass Email confirmation 1: ");

                    if (_userManager.Options.SignIn.RequireConfirmedAccount)
                    {
                        _logger.LogInformation("Go to RegisterConfirmation page");
                        return RedirectToPage("RegisterConfirmation", new { email = Input.Email, returnUrl = returnUrl });
                    }
                    else
                    {
                        await _signInManager.SignInAsync(user, isPersistent: false);
                        _logger.LogInformation("Dont require Confirmation anymore");
